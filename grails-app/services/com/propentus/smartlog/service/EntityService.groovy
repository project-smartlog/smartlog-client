...
package com.propentus.smartlog.service
...
class EntityService {
    ...
    ApiUser findApiUser(String authToken) {
        CouchDBProvider provider = new CouchDBProvider();
        ApiUserRepository repo = new ApiUserRepository(provider.getConnector());
        return repo.findByAuthToken(authToken);
    }

    ApiUser findApiUserBySHA1(String sha1) {
        CouchDBProvider provider = new CouchDBProvider();
        ApiUserRepository repo = new ApiUserRepository(provider.getConnector());
        return repo.findBySha1(sha1.toLowerCase());
    }
}
